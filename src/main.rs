...
impl App {
    async fn new(window: Arc<Window>) -> App {
        ...
        let image = image::load_from_memory(include_bytes!("awesomeface.png"))
            .unwrap()
            .flipv(); // image coords -> wgpu coords
        ...
        let config = surface
            .get_default_config(&adapter, texture_size.width, texture_size.height)
            .unwrap();
